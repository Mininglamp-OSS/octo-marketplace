...
| `OCTO_MARKETPLACE_CARD_ACTION_SECRET` | requires `OCTO_MARKETPLACE_INTERNAL_TOKEN` | empty | Shared HMAC secret octo-server uses to sign card-action callbacks (`POST /v1/card-actions/decide`). When empty, the callback route is permanently CLOSED (every request returns 401) rather than accepting unsigned input — that is the safe rollout path. Setting this WITHOUT `OCTO_MARKETPLACE_INTERNAL_TOKEN` FAILS BOOT: signatures would verify but operator-role authorization cannot happen, so every admin click 503s and retries into the DLQ. Setting `OCTO_MARKETPLACE_NOTIFY_TOKEN` without this secret produces a boot WARNING: cards are dispatched but every admin click is rejected (401). Match this against octo-server's `MARKETPLACE_CARD_ACTION_SECRET` / equivalent. |
