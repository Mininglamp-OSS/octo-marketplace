...
func TestDeleteExpertSoftDeletes(t *testing.T) {
	...
	mock.ExpectExec("UPDATE experts SET deleted_at = \\?, updated_at = \\? WHERE id = \\? AND deleted_at IS NULL").
		WithArgs(sqlmock.AnyArg(), sqlmock.AnyArg(), "e1").
		WillReturnResult(sqlmock.NewResult(0, 1))

	if err := repo.DeleteExpert(context.Background(), "e1", time.Now()); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatal(err)
...
func TestDeleteExpertAlreadyGone(t *testing.T) {
	...
	if err := repo.DeleteExpert(context.Background(), "e1", time.Now()); !errors.Is(err, ErrNotFound) {
		t.Fatalf("err = %v, want ErrNotFound", err)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatal(err)
