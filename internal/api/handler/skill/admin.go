...
func (h *Handler) AdminList(c *gin.Context) {
	...
	if err != nil {
		if !errors.Is(err, context.Canceled) && !errors.Is(err, context.DeadlineExceeded) {
			log.Printf("[skill] adminList failed: %q", err)
		}
...
func (h *Handler) AdminGet(c *gin.Context) {
	...
	if err != nil {
		...
		if !errors.Is(err, context.Canceled) && !errors.Is(err, context.DeadlineExceeded) {
			log.Printf("[skill] adminGet failed: %q", err)
		}
		apiresponse.Fail(c, http.StatusInternalServerError, errcode.InternalError, "internal error", nil, "")
		return
	}
...
func (h *Handler) AdminCreate(c *gin.Context) {
	...
	if err != nil {
		...
		if !errors.Is(err, context.Canceled) && !errors.Is(err, context.DeadlineExceeded) {
			log.Printf("[skill] adminCreate failed: %q", err)
		}
		apiresponse.Fail(c, http.StatusInternalServerError, errcode.InternalError, "internal error", nil, "")
		return
	}
...
func (h *Handler) AdminUpdate(c *gin.Context) {
	...
	if err != nil {
		...
		if !errors.Is(err, context.Canceled) && !errors.Is(err, context.DeadlineExceeded) {
			log.Printf("[skill] adminUpdate failed: %q", err)
		}
		apiresponse.Fail(c, http.StatusInternalServerError, errcode.InternalError, "internal error", nil, "")
		return
	}
...
func (h *Handler) AdminDelete(c *gin.Context) {
	...
	if err != nil {
		...
		if !errors.Is(err, context.Canceled) && !errors.Is(err, context.DeadlineExceeded) {
			log.Printf("[skill] adminDelete failed: %q", err)
		}
		apiresponse.Fail(c, http.StatusInternalServerError, errcode.InternalError, "internal error", nil, "")
		return
	}
...
func (h *Handler) AdminGetSkillMD(c *gin.Context) {
	...
	if err != nil {
		...
		if !errors.Is(err, context.Canceled) && !errors.Is(err, context.DeadlineExceeded) {
			log.Printf("[skill] adminGetSkillMD failed: %q", err)
		}
		apiresponse.Fail(c, http.StatusInternalServerError, errcode.InternalError, "internal error", nil, "")
		return
	}
...
func (h *Handler) AdminReupload(c *gin.Context) {
	...
	if err != nil {
		...
		if !errors.Is(err, context.Canceled) && !errors.Is(err, context.DeadlineExceeded) {
			log.Printf("[skill] adminReupload failed: %q", err)
		}
		apiresponse.Fail(c, http.StatusInternalServerError, errcode.InternalError, "internal error", nil, "")
		return
	}
