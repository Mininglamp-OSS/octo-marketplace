...
// @Description Provision a squad into the chosen workspace/runtime: install each member as a Loop agent (with its skills), create the squad led by the leader member, write its strategies as instructions, and attach the rest. Aggregates octo-fleet calls on behalf of the caller and rolls back on partial failure.
// @Tags expert
// @ID squad.install
// @Accept json
// @Produce json
